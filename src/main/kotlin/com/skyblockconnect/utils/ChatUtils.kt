...
import com.skyblockconnect.SkyblockConnect.scope
import com.skyblockconnect.event.EventBus.register
import com.skyblockconnect.event.EventListener
import com.skyblockconnect.event.impl.PacketEvent
...
object ChatUtils {
    ...
    fun modMessage(msg: Any?) = chat("${SkyblockConnect.PREFIX} $msg")

    fun debug(flag: String, msg: Any?) {
        if (SkyblockConnect.debugFlags.contains(flag)) modMessage(msg)
    }
    ...
    fun clickableChat(message: String, prefix: Boolean = false, command: String? = null, hover: String? = null, copy: String? = null) {
        ...
        ChatUtils.chat(if (prefix) Component.literal(SkyblockConnect.PREFIX + " ").append(mainComponent) else mainComponent)
    }
