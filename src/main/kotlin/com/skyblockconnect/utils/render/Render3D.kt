...
object Render3D {
    fun renderBlock(
        ...
        if (fill) addFilledBoxVertices(
            ...
            ctx.consumers.getBuffer(if (phase) ModRenderLayers.FILLED_THROUGH_WALLS else ModRenderLayers.FILLED),
        ...
        if (outline) renderLineBox(
            ctx.matrixStack.last(),
            ctx.consumers.getBuffer(if (phase) ModRenderLayers.LINES_THROUGH_WALLS else ModRenderLayers.LINES),
            minX, minY, minZ, maxX, maxY, maxZ,
    ...
    fun renderCircle(
        ...
        ctx.matrixStack.pushPose()
        ctx.matrixStack.translate(ctx.camera.position().reverse())
        val buffer = ctx.consumers.getBuffer(if (phase) ModRenderLayers.CIRCLE_FILLED_THROUGH_WALLS else ModRenderLayers.CIRCLE_FILLED)
    ...
    fun renderTracer(ctx: RenderContext, point: Vec3, color: Color, thickness: Number = 2.5) {
        ...
        val buffer = (ctx.consumers as MultiBufferSource.BufferSource).getBuffer(ModRenderLayers.LINES_THROUGH_WALLS)
        ...
        ctx.consumers.endBatch(ModRenderLayers.LINES_THROUGH_WALLS)
        ctx.matrixStack.popPose()
    }
