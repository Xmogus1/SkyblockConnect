...
object WorldUtils {
    fun getStateAt(pos: BlockPos) = SkyblockConnect.mc.level?.getBlockState(pos) ?: Blocks.AIR.defaultBlockState()
    ...
    fun setBlockAt(pos: BlockPos, state: BlockState) = SkyblockConnect.mc.level?.setBlock(pos, state, 19)

    fun isChunkLoaded(x: Int, z: Int): Boolean {
        return SkyblockConnect.mc.level?.getChunk(x shr 4, z shr 4, ChunkStatus.FULL, false) != null
    }

    fun getBlockEntityList(): List<BlockPos> {
        val player = SkyblockConnect.mc.player ?: return emptyList()
        val level = SkyblockConnect.mc.level ?: return emptyList()
        val renderDistance = SkyblockConnect.mc.options.renderDistance().get()
