...
import com.skyblockconnect.event.impl.MouseClickEvent
import com.skyblockconnect.event.impl.PlayerInteractEvent
import com.skyblockconnect.utils.ChatUtils
import com.skyblockconnect.utils.remove
...
abstract class Event(val cancelable: Boolean = false) {
    ...
    open var isCanceled = false
        set(value) {
            ...
            if (value && ! field && SkyblockConnect.debugFlags.contains("cancel")) {
                captureSource()
            }
            field = value
        }
    ...
    private fun captureSource() {
        ...
        for (i in 3 until stack.size) {
            ...
            if (className.startsWith("com.skyblockconnect.event") ||
                className.startsWith("java.lang") ||
                className.startsWith("kotlin.") ||
                className.contains("EventBus")
            ) continue

            val eventName = this.javaClass.name.remove("com.skyblockconnect.event.impl.")
