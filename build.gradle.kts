...
dependencies {
    ...
    // Hypixel Mod API core — classes provided at runtime by the HypixelModAPI mod (compile-only here).
    compileOnly(files("libs/mod-api-1.0.2.jar"))
...
listOf("jarCheat", "jarLegit", "jarCurseforge").forEach { taskName ->
    tasks.named<Jar>(taskName) {
        // SkyblockConnect-<mod_version>-<minecraft_version>.jar
...
// SBC ships one build, so jarLegit is *the* jar — no legit/cheat split to label.
tasks.named<Jar>("jarLegit") { archiveClassifier.set("") }
